    ...
    connect: {
      options: {
        ...
        middleware: function(connect, options, middlewares) {
          var rules = ["!\\.html|\\.json|\\.js|\\.css|\\.svg|\\.jp(e?)g|\\.png|\\.gif$ /index.html"];
      ...
      livereload: {
        options: {
          open: true,
          middleware: function(connect, options, middlewares) {
            var rules = ["!\\.html|\\.json|\\.js|\\.css|\\.svg|\\.ttf|\\.woff|\\.jp(e?)g|\\.png|\\.gif$ /index.html"];
            middlewares.unshift(rewrite(rules));
            var connectApp = connect();
            middlewares = middlewares.concat([
              connectApp.use(serveStatic('.tmp')),
              connectApp.use(serveStatic('/bower_components')),
              connectApp.use(serveStatic('/node_modules')),
              connectApp.use(serveStatic('/app/styles')),
              connectApp.use(serveStatic(appConfig.app))
            ]);
            return middlewares;
          }
        }
      ...
      dist: {
        options: {
          ...
          port: 9004,
          middleware: function(connect, options, middlewares) {
            var rules = ["!\\.html|\\.json|\\.js|\\.xml|\\.css|\\.svg|\\.ttf|\\.woff|\\.jp(e?)g|\\.png|\\.gif$ /index.html"];
            middlewares.unshift(rewrite(rules));
            return middlewares;
          },
          base: '<%= yeoman.dist %>'
